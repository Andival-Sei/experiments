...
- Глобальные стили подключаются из `src/index.scss`, а компонентные — через CSS Modules (например, `src/App.module.scss`, `src/components/Header/Header.module.scss`).
...
- `src/components/Header/Header.tsx` — адаптивный шапка с брендированием, навигацией и CTA. На ширине до `900px` меню превращается в выпадающий список.
- Герой-блок (`.hero`) и последующие секции (`#features`, `#technology`, `#pricing`, `#faq`, `#contact`) описаны прямо в `src/App.tsx` и стилизуются в `src/App.module.scss`.
- Дизайн-токены и фоновые градиенты определены в `src/index.scss`. Меняйте значения переменных `--accent`, `--layout-max-width` и т.д., чтобы адаптировать тему под бренд.
- Для навигации по секциям используются якоря — добавляйте новые пункты меню, создавая соответствующие `section` с `id` и оформляя их в `App.module.scss` или вынося в отдельные компоненты.
- В Header встроен переключатель темы (`ThemeProvider` + `useTheme`), который по умолчанию подстраивается под системные настройки пользователя и сохраняет выбор в `localStorage`.

## Темизация

- Источник правды по теме — `ThemeProvider` (`src/theme/theme-provider.tsx`). Он считывает системную настройку (`prefers-color-scheme`), восстанавливает пользовательский выбор из `localStorage` и управляет атрибутом `data-theme` на `<html>`.
- Контекст и хук находятся в `src/theme/theme-context.ts`. Подключайте `useTheme()` в компонентах, если нужно переключать или отображать текущую тему.
- CSS-переменные для светлой и тёмной темы описаны в `src/index.scss`. При смене темы меняются токены (`--text-*`, `--surface-*`, `--page-gradient`), поэтому все компоненты, использующие их, автоматически подстраиваются.
- Чтобы добавить собственные токены, объявите их в светлой и тёмной секции и используйте через `var(--your-token)` в SCSS-модулях.
